...
1. [刨根究底字符编码 - 笨笨阿林 - 博客园 (cnblogs.com)](https://www.cnblogs.com/benbenalin/category/1005679.html)
...
抽象字符是文本处理中的最小信息单位，用于表示字母、数字、标点符号、控制字符以及表情符号等，绝大多数是可见字符。

简体汉字、繁体汉字、拉丁字符、罗马数字、阿拉伯数字、古印度文字、楔形文字、甲骨文等等，都属于抽象字符。

在没有计算机之前，这些字符仍然广泛使用。

随着时代的发展，目前互联网常见的各类表情符号，也纳入了抽象字符的范围。


### 抽象字符编号

计算机使用电路开关来处理二进制 0、1 数字，为了使计算机可以表示抽象字符，人们开始对常见的字符进行编号。编号为非负整数，使其可以被二进制表示。

最早使用的编号为 ASCII 码：使用 8 位二进制（0x00 - 0xFF）表示至多 256 个字符，可以表达几乎所有的英文字符。
...
在 unicode 标准中对抽象字符的编号也被称为码点（Code Point），
...
* GB18030：国标，最新的中文字符集，包含 70244 个中文、繁体中文、少数民族文字、生僻字以及日韩文字等。长度为 1 个、2 个或 4 个字节。

* Unicode 字符集：统一码、标准万国码，世界范围内通用的字符集，由专门的机构维护，目前仍在持续更新中。其使用 4 个字节的长度表示一个字符，范围从 U+00000000 到 U+0010FFFF。
...
抽象字符编号可以就是字符集编码，例如 ASCII 编码。也可以是经过一定规则转换后的编码，例如 unicode 字符集的 utf-8 编码、utf-16 编码。

将编码后的计算机二进制字节流，按照一定的解码方式，转换为抽象字符编号的过程，叫做解码。


### 字形码

抽象字符在屏幕显示或打印机打印时，需要根据字体映射为字符点阵。

字形码是字体文件中存储的字符图形数据（如点阵或矢量图形），用于在屏幕或打印机上渲染抽象字符的外观。

可以使用 16×16、24×24、48×48 等字符点阵表示出抽象字符。我们以 16×16 的点阵示意，使用 excel 模拟字符点阵：

![image-20211020163951054](/images/techs/character-set/image-20211020163951054.png)
...
> [!note]
> * 字符集层（抽象字符 -> 码点）：解决“有什么字符”和“如何编号”的问题。
> 
> * 编码层（码点 -> 字节序列）：解决“如何存储和传输”的问题。
> 
> * 渲染层（码点 -> 字形）：解决“如何显示”的问题。操作系统和字体文件根据码点找到对应的字形信息，在屏幕上绘制出抽象字符的图形。
...
该标准与 Unicode 标准的映射关系较为混乱，有连续的，也有非连续的，需要逐一处理来建立映射关系。
...
![image-20220804161625114](/images/techs/character-set/image-20220804161625114.png)

![image-20220804161654868](/images/techs/character-set/image-20220804161654868.png)

![image-20220804162015392](/images/techs/character-set/image-20220804162015392.png)

![image-20220804162035166](/images/techs/character-set/image-20220804162035166.png)


详细的转换算法，可以参考 [《GB18030与Unicode编码转换算法.pdf》]
